...
def player(board):
    ...
    # Given: In the initial game state, X gets the first move. Subsequently, the player alternates with each additional move.
    moves = 0
    for row in board:
        for cell in row:
            if cell == X or cell == O:
...
def minimax(board):
    ...
    if player(board=board) == X:
        ...
        for action in actions(board=board):
            ...
            if minimax_value(board) > max_score:
                best_action = action 
        return best_action
    ...
    if player(board=board) == O:
        ...
        for action in actions(board=board):
            new_board = result(board=board, action=action)
            if minimax_value(board) < min_score:
                best_action = action 
        return best_action
